...
export default function VerifyEmailPage() {
  ...
  useEffect(() => {
    ...
    const verifyToken = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/verify-email?token=${token}`,
          {
            method: "GET",
            credentials: "include",
            headers: {
        ...
        if (contentType && contentType.includes("application/json")) {
          ...
          // Handle HTML response (the backend might be returning a redirect page)
          if (response.ok) {
            setStatus("success")
    ...
    verifyToken()
  }, [token, router, toast, refreshAuthState])
