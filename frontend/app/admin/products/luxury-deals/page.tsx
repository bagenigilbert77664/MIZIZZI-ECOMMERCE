...
export default function LuxuryDealsPage() {
  const router = useRouter()
  const { isAuthenticated, isLoading: authLoading } = useAdminAuth()
  const isMobile = useMobile()

  // State for products and loading
  const [luxuryProducts, setLuxuryProducts] = useState<any[]>([])
  ...
  // Fetch luxury deal products
  const fetchLuxuryProducts = useCallback(async () => {
    ...
    try {
      ...
      console.log("Fetching luxury deal products...")
      const response = await adminService.getProducts({ luxury_deal: true, per_page: 10000 })

      const fetchedProducts = response.items || []
      console.log(`Successfully fetched ${fetchedProducts.length} luxury deal products`)

      setLuxuryProducts(fetchedProducts)
      ...
      console.error("Error fetching luxury deal products:", error)
      setError(error.message || "Failed to load luxury deal products. Please try again.")
      toast({
        title: "Error",
        description: "Failed to load luxury deal products. Please try again.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  }, [isAuthenticated, fetchLuxuryProducts])
  ...
  const toggleSelectAll = () => {
    if (selectedProducts.length === luxuryProducts.length) {
      setSelectedProducts([])
    } else {
      setSelectedProducts(luxuryProducts.map((product) => product.id.toString()))
    }
  }
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      ...
      setLuxuryProducts((prev) => prev.filter((p) => p.id.toString() !== productToDelete))
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      if (successfulDeletes.length > 0) {
        setLuxuryProducts((prev) => prev.filter((p) => !successfulDeletes.includes(p.id.toString())))
      }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-gradient-to-r from-purple-500/20 to-purple-700/20 rounded-xl p-6 shadow-sm border">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-2xl font-bold tracking-tight flex items-center">
                <Crown className="mr-2 h-5 w-5 text-purple-600" /> Luxury Deal Products
              </h1>
            </div>
            <p className="text-muted-foreground">Manage premium products with exclusive luxury deals</p>
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
            ...
            <Button
              onClick={() => router.push("/admin/products/new?preset=luxury_deal")}
              className="shadow-sm transition-all duration-200 bg-purple-600 hover:bg-purple-700 text-white"
              size={isMobile ? "sm" : "default"}
            >
              <Plus className="mr-1 h-4 w-4" /> {isMobile ? "Add" : "Add Luxury Product"}
            </Button>
          </div>
        </div>
      </div>
      ...
      {/* Stats Card */}
      <div className="bg-gradient-to-r from-purple-600 to-purple-800 text-white rounded-xl shadow-md p-6">
        <div className="flex items-center gap-3 mb-2">
          <div className="bg-white/20 p-2 rounded-lg">
            <Diamond className="h-5 w-5" />
          </div>
          <h2 className="text-lg font-semibold">Luxury Deals Overview</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
          <div className="bg-white/10 rounded-lg p-4">
            <div className="text-sm opacity-80 mb-1">Total Luxury Products</div>
            <div className="text-2xl font-bold">{luxuryProducts.length}</div>
          </div>
          <div className="bg-white/10 rounded-lg p-4">
          ...
          <div className="bg-white/10 rounded-lg p-4">
            <div className="text-sm opacity-80 mb-1">Average Price</div>
            <div className="text-2xl font-bold">
              KSh{" "}
              {luxuryProducts.length > 0
                ? Math.round(
                    luxuryProducts.reduce((acc, product) => acc + (product.sale_price || product.price), 0) /
                      luxuryProducts.length,
                  ).toLocaleString()
                : 0}
            </div>
          ...
          {isLoading ? (
            ...
            <div className="p-6">
              <div className="flex flex-col items-center justify-center py-8 text-center rounded-xl">
                ...
                <h3 className="text-lg font-medium mb-2">Failed to load luxury products</h3>
            ...
            <div className="p-6">
              <div className="flex flex-col items-center justify-center py-8 text-center rounded-xl">
                ...
                <h3 className="text-lg font-medium mb-2">No luxury products found</h3>
                <p className="text-muted-foreground mb-4">
                  {searchQuery
                    ? "Try adjusting your search to see more products"
                    : "Get started by adding your first luxury product"}
                </p>
                {searchQuery ? (
                ...
                >
                  <Plus className="mr-2 h-4 w-4" /> Add Luxury Product
                </Button>
              </div>
            </div>
          ) : (
                          ...
                          <div className="flex items-center">
                            ...
                            <div className="ml-3 flex items-center">
                              <div className="h-16 w-16 rounded-md border overflow-hidden bg-background p-1 mr-3 relative">
                                ...
                                />
                                <Badge className="absolute top-0 right-0 bg-purple-600 text-white text-[10px] px-1 py-0 rounded-sm">
                                  Luxury
                                </Badge>
                              </div>
                              <div>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Delete Luxury Product</DialogTitle>
            <DialogDescription>
      ...
      <Dialog open={isBulkDeleteDialogOpen} onOpenChange={setIsBulkDeleteDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Delete Luxury Products</DialogTitle>
            <DialogDescription>
