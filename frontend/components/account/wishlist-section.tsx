...
export function WishlistSection() {
  ...
  const handleRemoveFromWishlist = async (id: string) => {
    ...
    // Mark item as being removed for immediate UI feedback
    setRemovedItems((prev) => [...prev, id])
    ...
    try {
      ...
      // Force refresh after a short delay to ensure database sync
      setTimeout(() => {
        refreshWishlist()
      }, 500)
    } catch (error) {
      ...
      // Remove from the removed items list if there was an error
  ...
  const handleClearWishlist = async () => {
    ...
    try {
      ...
      // Mark all items as removed for immediate UI feedback
      setRemovedItems(wishlist.map((item) => String(item.id)))
  ...
  const filteredWishlist = wishlist.filter((item) => !removedItems.includes(String(item.id)))

  console.log(
    "[v0] 🖼️ Wishlist items with images:",
    ...
    <div>
      ...
      {isUpdating && !isRefreshing ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredWishlist.map((item) => {
            console.log(`[v0] 🖼️ Rendering wishlist item ${item.id} (${item.name}) with image:`, item.image)
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  disabled={loadingItems[String(item.id)]}
                ...
                <Link href={`/product/${item.slug || item.id}`} className="relative aspect-square overflow-hidden">
                  <Image
                    src={item.image || "/placeholder.svg?height=300&width=300&query=product"}
                    alt={item.name}
                    fill
                    className="object-cover transition-transform duration-300 group-hover:scale-105"
                    onError={(e) => {
                      console.error(`[v0] ❌ Failed to load image for ${item.name}:`, item.image)
                      // Set fallback image
                      e.currentTarget.src = "/diverse-products-still-life.png"
                    }}
                    onLoad={() => {
                      console.log(`[v0] ✅ Successfully loaded image for ${item.name}:`, item.image)
                    }}
                ...
                <div className="p-3 flex flex-col flex-1">
                  ...
                  <div className="mt-auto pt-2">
                    <Button
                      ...
                      size="sm"
                      className="w-full"
                      onClick={() => handleAddToCart(String(item.id), item.name)}
                      disabled={loadingItems[String(item.id)] || isCartUpdating}
                    >
                      {loadingItems[String(item.id)] || isCartUpdating ? (
