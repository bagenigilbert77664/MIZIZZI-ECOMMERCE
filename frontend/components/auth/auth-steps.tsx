...
export function AuthSteps() {
  ...
  useEffect(() => {
    if (userId && flow === "register" && step === "register") {
      console.log("[v0] UserId set during registration, navigating to verification step")
      setStep("verification")
    }
  }, [userId, flow, step])
  ...
  const handleRegisterSubmit = async (name: string, password: string) => {
    setIsLoading(true)
    try {
      console.log("[v0] Starting registration for:", identifier)
      ...
      if (response.user_id) {
        console.log("[v0] User ID received:", response.user_id)
        ...
        console.log("[v0] Verification state stored in localStorage")

        // Send verification code
        try {
          console.log("[v0] Sending verification code to:", identifier)
          await authService.sendVerificationCode(identifier)
          ...
          console.log("[v0] Navigating to verification step")
          setStep("verification")
        } catch (verificationError: any) {
          ...
          // User can resend the code from the verification page
          setStep("verification")
        }
      } else {
        console.log("[v0] No user ID in response, showing welcome screen")
        // Show welcome screen
        setStep("welcome")
      ...
      setIsLoading(false)
      console.log("[v0] Registration process completed")
    }
  }
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      // Handle server errors with more helpful messaging
      if (error.message?.includes("Server error") || error.message?.includes("email service")) {
        errorMessage = error.message
        toastDuration = 8000 // Longer duration for server errors
  ...
  if (step === "verification") {
    console.log("[v0] Rendering verification step for:", identifier)
    return (
  ...
  if (step === "register") {
    console.log("[v0] Rendering register step for:", identifier)
    return (
