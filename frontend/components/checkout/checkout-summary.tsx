...
export default function CheckoutSummary({
  ...
  if (isLoading) {
    return (
      <Card className="bg-white shadow-sm border border-gray-200 rounded-md overflow-hidden sticky top-6">
              ...
              {items.map((item) => (
                <div key={item.id} className="flex gap-3 py-2 border-b border-gray-100 last:border-0">
                  <div className="relative h-16 w-16 flex-shrink-0 overflow-hidden rounded-md border border-gray-200 bg-gray-50">
                    {item.product.thumbnail_url || (item.product.image_urls && item.product.image_urls.length > 0) ? (
                      <Image
