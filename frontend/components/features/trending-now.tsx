...
export function TrendingNow() {
  ...
  const fetchTrending = useCallback(async () => {
    try {
      ...
      setError(null)
      const res =
        (await productService.getProducts({ limit: 12, sort_by: "popularity", sort_order: "desc" })) ||
        (await productService.getProducts({ limit: 12, sort_by: "price", sort_order: "desc" }))
      ...
      try {
        const fallback = await productService.getProducts({ limit: 12 })
        if (fallback) {
          const processedProducts = fallback.map((product) => ({
            ...product,
            image_urls: (product.image_urls || []).map((url) => {
              // If it's a Cloudinary public ID, generate URL:
              if (typeof url === "string" && !url.startsWith("http")) {
