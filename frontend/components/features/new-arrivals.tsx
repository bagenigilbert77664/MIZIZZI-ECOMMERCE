...
export function NewArrivals() {
  ...
  const fetchNew = useCallback(async () => {
    try {
      ...
      setError(null)
      const res =
        (await productService.getProducts({ limit: 6, sort_by: "created_at", sort_order: "desc" })) ||
    ...
    <section className="w-full mb-8">
      <div className="bg-cherry-900 text-white flex items-center justify-between px-4 py-2">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm sm:text-base font-bold whitespace-nowrap">New Arrivals</h2>
        </div>
        <Link href="/products?sort=newest" className="flex items-center gap-1 text-xs sm:text-sm font-medium hover:underline">
          See All
        ...
        <div className="grid grid-cols-2 gap-[1px] bg-gray-100 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          <AnimatePresence mode="popLayout">
            {products.map((product, i) => (
              ...
              >
                <PromoProductCard product={product} badgeText="New" />
              </motion.div>
            ))}
          </AnimatePresence>
        </div>
