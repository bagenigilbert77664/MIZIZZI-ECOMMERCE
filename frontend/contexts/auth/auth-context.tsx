...
// Token key constants to ensure consistency with auth service
const TOKEN_KEYS = {
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const checkAuth = async (): Promise<boolean> => {
    try {
      ...
      // First check if we have BOTH tokens
      const hasAccessToken = !!localStorage.getItem(TOKEN_KEYS.ACCESS_TOKEN)
      const hasRefreshToken = !!localStorage.getItem(TOKEN_KEYS.REFRESH_TOKEN)

      if (!hasAccessToken || !hasRefreshToken) {
        console.log("Missing tokens, user is not authenticated", {
          hasAccessToken,
          hasRefreshToken,
        })
        setUser(null)
  ...
  const register = async (credentials: {
    ...
    try {
      ...
      if (error.field) {
        throw error // Preserve field-specific errors for the form
      } else {
        toast({
  ...
  const logout = async (): Promise<void> => {
    try {
      ...
      // Explicitly clear all auth data
