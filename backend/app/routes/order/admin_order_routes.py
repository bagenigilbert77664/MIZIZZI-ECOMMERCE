...
def format_order_for_response(order, include_items=False):
    ...
    try:
        ...
        # Always include user field as dict or None (never int)
        user_data = None
        if order.user_id:
            try:
        ...
        logger.error(f"Error formatting order {getattr(order, 'id', None)}: {str(e)}")
        # Defensive: always include user_id, even if order is None
    ...
    try:
        ...
        for order in paginated.items:
            ...
            if "user_id" not in order_data:
                order_data["user_id"] = getattr(order, 'user_id', None)
...
def get_order_stats():
    ...
    try:
        ...
        if period == 'day':
            ...
            start_date = datetime(today.year, 1, 1, 0, 0, 0, tzinfo=timezone.utc)
            # Use extract to get month
            group_by = func.extract('month', Order.created_at).cast(db.Integer)
            label_format = '%m'
        else:  # month (default)
            # Get sales for each day of the month
            start_date = datetime(today.year, today.month, 1, 0, 0, 0, tzinfo=timezone.utc)
...
def export_orders():
    ...
    try:
        ...
        for order in orders:
            ...
            order_items = OrderItem.query.filter_by(order_id=order.id).all()
            items_text = ", ".join([
                f"{item.quantity}x {db.session.get(Product, item.product_id).name if db.session.get(Product, item.product_id) else 'Unknown Product'}"
                for item in order_items
            ])
