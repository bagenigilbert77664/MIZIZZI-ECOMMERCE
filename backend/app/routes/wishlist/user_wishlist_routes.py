...
def clear_user_cache(user_id: int):
    ...
    try:
        ...
        # Clear wishlist check cache for common product IDs
        # In a production environment, you'd want to use Redis pattern matching
        # For now, we'll clear a reasonable range of product IDs
        for product_id in range(1, 1001):  # Clear cache for products 1-1000
            cache.delete(f"wishlist_check_{user_id}_{product_id}")

        # Clear general wishlist cache patterns
        # This is a brute force approach for testing - in production use Redis SCAN or pattern matching
        for page in range(1, 21):  # Clear first 20 pages
            for per_page in [10, 20, 50, 100]:
                for sort_by in ['created_at', 'product_name', 'product_price']:
                    for sort_order in ['asc', 'desc']:
                        # Clear cache for different filter combinations
                        for filters_hash in [hash(str([])), hash(str([('in_stock_only', True)])),
                                           hash(str([('on_sale_only', True)])), hash(str([('featured_only', True)]))]:
                            cache_key = f"wishlist_{user_id}_{page}_{per_page}_{filters_hash}_{sort_by}_{sort_order}"
                            cache.delete(cache_key)

        # Also try to clear the Flask-Caching cache entirely for this user's data
        # This is more aggressive but ensures cache consistency
        if hasattr(cache, 'cache') and hasattr(cache.cache, '_cache'):
            # For simple cache backends, try to clear keys that contain the user_id
            try:
                cache_keys_to_delete = []
                if hasattr(cache.cache._cache, 'keys'):
                    for key in list(cache.cache._cache.keys()):
                        if isinstance(key, str) and str(user_id) in key:
                            cache_keys_to_delete.append(key)

                for key in cache_keys_to_delete:
                    cache.delete(key)
            except:
                pass  # Ignore errors in cache key enumeration
...
def get_wishlist_query(user_id: int, filters: Dict = None):
    ...
    """
    query = db.session.query(WishlistItem).join(Product).join(Category).join(Brand).filter(
...
def get_wishlist():
    ...
    Query Parameters:
        ...
        - sort_order: Sort order (asc, desc)
        - _cache_bust: Cache busting parameter
    """
    if request.method == 'OPTIONS':
        return '', 200
    ...
    try:
        ...
        cache_bust = request.args.get('_cache_bust')
        ...
        # Cache result (unless cache busting was requested)
        if not cache_bust:
...
def get_wishlist_stats():
    ...
    try:
        ...
        cached_stats = None if cache_bust else cache.get(cache_key)

        if cached_stats:
            return jsonify(cached_stats), 200

        # Get all wishlist items with product details
        wishlist_items = db.session.query(WishlistItem).join(Product).join(Category).join(Brand).filter(
        ...
        if not wishlist_items:
            ...
            # Cache empty stats for shorter time (unless cache busting was requested)
            if not cache_bust:
        ...
        # Cache stats (unless cache busting was requested)
        if not cache_bust:
...
def export_wishlist():
    ...
    try:
        ...
        # Get wishlist items with proper error handling
        wishlist_items = db.session.query(WishlistItem).join(Product).join(Category).join(Brand).filter(
            WishlistItem.user_id == current_user_id,
            Product.is_active == True
