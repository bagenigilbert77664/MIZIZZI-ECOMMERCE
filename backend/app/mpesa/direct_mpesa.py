"""
Direct M-PESA Integration Module for Mizizzi E-commerce Platform.
...
def validate_amount(amount: Union[int, float, str]) -> int:
    """
    ...
    try:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                # Make request
                response = requests.post(STK_PUSH_URL, headers=headers, json=payload, timeout=REQUEST_TIMEOUT)
...
def query_stk_status(checkout_request_id: str) -> Dict[str, Any]:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            # Make request
            response = requests.post(QUERY_URL, headers=headers, json=payload, timeout=REQUEST_TIMEOUT)
...
def process_stk_callback(callback_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
...
def simulate_callback(
    ...
    if success:
        ...
        # Generate metadata for successful transaction
        metadata_items = [
            {"Name": "Amount", "Value": amount or 1.0},
            {"Name": "MpesaReceiptNumber", "Value": f"MP{random.randint(100000000, 999999999)}"},
            {"Name": "TransactionDate", "Value": int(datetime.now().timestamp())},
            {"Name": "PhoneNumber", "Value": phone_number or "254712345678"}
        ]
    else:
        result_code = 1032
    ...
    callback_data = {
        "Body": {
            "stkCallback": {
                "MerchantRequestID": f"29115-34620561-{random.randint(1, 999)}",
                "CheckoutRequestID": checkout_request_id,
    ...
    logger.info(f"Simulated callback: {json.dumps(callback_data, indent=2)}")
    return callback_data

def test_module() -> bool:
    """
    Test the M-PESA module functionality.

    Returns:
        bool: True if module is working, False otherwise
    """
    try:
        logger.info("Testing M-PESA module...")
        ...
        test_phones = ["0712345678", "712345678", "254712345678", "+254712345678"]
        for phone in test_phones:
            try:
                formatted = format_phone_number(phone)
                if not formatted.startswith("254") or len(formatted) != 12:
                    logger.error(f"Phone formatting test failed for {phone}")
                    return False
            except Exception as e:
                logger.error(f"Phone formatting test failed for {phone}: {str(e)}")
                return False

        logger.info("Phone formatting test passed")
        ...
        try:
            ...
            logger.error(f"Amount validation test failed: {str(e)}")
            return False

        # Test simulation
        try:
            sim_response = simulate_stk_push("254712345678", 1, "TEST")
            if not sim_response.get("CheckoutRequestID"):
                logger.error("Simulation test failed")
                return False
            logger.info("Simulation test passed")
        except Exception as e:
            logger.error(f"Simulation test failed: {str(e)}")
            return False

        logger.info("All M-PESA module tests passed")
        return True
...
def get_module_info() -> Dict[str, Any]:
    """
    Get information about the M-PESA module.

    Returns:
        dict: Module information
    """
    return {
        "module": "Direct M-PESA Integration",
        "version": "2.0.0",
        ...
        "status": "active" if test_module() else "inactive"
    }

# For backward compatibility
def mock_query_stk_status(checkout_request_id: str) -> Dict[str, Any]:
    """
    ...
    # Generate a mock response based on current time
